...
def generate_reservoir(size, radius, degree):
    ...
    A = sparse.rand(size, size, density=sparsity, random_state=42).todense()
...
np.savetxt('data/Expansion_2step_back' + 'R_size_train_' + str(res_params['train_length']) + '_Rd_' + str(
    res_params['radius']) + '_Shift_' + str(shift_k) + '.txt', output)
